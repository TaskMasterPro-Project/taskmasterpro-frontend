...
import { createComment, deleteComment, editComment, getCommentsForTask } from '../../axios/apiClient';
...
const CommentsSection: React.FC<commentSectionProps> = ({selectedProject, taskId}) => {
  ...
  const handleCommentOptionsClick = (commentId: number) => (event: React.MouseEvent<HTMLElement>) => {
  ...
  function handleStartEditingComment(commentId: number, commentContent: string){
  ...
  const handleEditComment = async (commentId: number) => {
    ...
    await editComment(selectedProject.id, taskId, commentId, commentContent)
      ...
      <CardContent>
        ...
        {comments.map((comment) => (
            ...
            action={
              <Box>
                <IconButton
                  ...
                  aria-haspopup="true"
                  onClick={handleCommentOptionsClick(comment.id)}
                  sx={{ml: '5px'}}>
                  <MoreVertIcon />
                </IconButton>
                <Menu
                  ...
                  onClose={handleClose}>
                  <MenuItem onClick={() => handleStartEditingComment(comment.id, comment.content)}>Edit</MenuItem>
                  <MenuItem style={{ color: 'red' }} onClick={() => handleDeleteComment(comment.id)}>Delete</MenuItem>
                </Menu>
              </Box>
            }
            ...
            subheader={
              ...
              <TextField 
                inputRef={comment.id === editingCommentId ? editInputRef : null}
